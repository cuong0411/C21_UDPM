...
AS
UPDATE CTHD
SET KHUYENMAI = IIF(SL > 500, 0.1, (IIF(SL > 100, 0.05, 0)))
FROM CTHD
GO
	--CASE
...
CREATE PROCEDURE p8
AS
SELECT vt.MAVT, SUM(vt.SLTON - ct.SL) AS [SO LUONG TON]
FROM VATTU vt, CTHD ct
WHERE vt.MAVT = ct.MAVT
GROUP BY vt.MAVT
GO
...
AS
SELECT ct.MAHD,
	SUM(ct.SL * ct.GIABAN *(1-  KHUYENMAI)) AS [TONG TRI GIA SAU KHUYEN MAI]
...
AS
    ...
    (
        ...
        GROUP BY kh.MAKH
        HAVING SUM(ct.SL * ct.GIABAN *(1-  KHUYENMAI)) > @x
    )
GO
EXEC p10 10000000
