...
int linearSearchHoTen(HS a[], int n, char x[]);

int linearSearchDTB(HS a[], int n, float dtb);
...
int main()
{
    ...
    */
    ...
    /*
    float dtb;
    printf("Nhap diem trung binh de tim: ");
    scanf("%f", &dtb);
    int vt = linearSearchDTB(lopHoc, soHocSinh, dtb);
    if (vt == -1)
    {
    	printf("\nKhong co hs\n");
	}
    ...
    */
    ...
    gets(hoTen);
    int vt = linearSearchHoTen(lopHoc, soHocSinh, hoTen);
    if (vt == -1)
    {
    	printf("\nKhong co hs\n");
	}
...
int linearSearchDTB(HS a[], int n, float x)
{
	for (int i = 0; i < n; i++)
	{
		if (x == a[i].diemTrungBinh)
		{
			return i;
		}
	}
